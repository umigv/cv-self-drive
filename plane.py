...
def ground_plane(
    depths, iters: int = 60, kernel: tuple[int, int] = (1, 12), tol: float = 0.15
):
...
# >>> change: new function that uses an external mask (from RightTurn) instead of internal HSV
def external_mask_and_ransac(external_mask, depths):
    ground_mask, coeffs = ground_plane(depths)

    # Resize external_mask to match depth dimensions if they differ
    dh, dw = depths.shape[:2]
    mh, mw = external_mask.shape[:2]
    if (mh, mw) != (dh, dw):
        external_mask = cv2.resize(external_mask, (dw, dh), interpolation=cv2.INTER_NEAREST)

    # Treat the external mask as the driveable region, intersect with ground plane
    ext_bool = external_mask > 127
    driveable = (ground_mask & ext_bool).astype(np.uint8) * 255
